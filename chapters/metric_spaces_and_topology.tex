...
\begin{definition}[Continuity on a metric space]
...
\begin{definition}[Open set, closed set and limit point]
...
%     \begin{enumerate}
%         \item A typical $\varepsilon$-neighborhood ($V_\varepsilon(x)$) is an open set. \\
%         From the definition, if $(X, d)$ is a metric space then $V_\varepsilon(x) = \{ v \in X: d(x, v) < \varepsilon\}$ with $x \in X$. Let $v \in V_\varepsilon(x)$ then define $W_{\varepsilon'}(v) = \{w \in V_\varepsilon(x) : d(w, v) < \varepsilon'\}$. $V$ is open if $W \subseteq V$. 
%         \item B
%     \end{enumerate}
% \end{eg}
